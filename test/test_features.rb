...
class TestFeatures < Minitest::Test
  ...
  def test_delete_feature
    with_app do |app_data|
      heroku.post_feature('user-env-compile', app_data['name'])
      response = heroku.delete_feature('user-env-compile', app_data['name'])
  ...
  def test_delete_feature_app_not_found
    assert_raises(Heroku::API::Errors::RequestFailed) do
      heroku.delete_feature('user-env-compile', random_name)
    end
  end
  ...
  def test_get_feature
    with_app do |app_data|
      response = heroku.get_feature('user-env-compile', app_data['name'])
  ...
  def test_post_feature
    with_app do |app_data|
      response = heroku.post_feature('user-env-compile', app_data['name'])
  ...
  def test_post_feature_app_not_found
    assert_raises(Heroku::API::Errors::NotFound) do
      heroku.post_feature('user-env-compile', random_name)
    end
  end
